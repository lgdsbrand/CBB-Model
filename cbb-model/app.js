/* =========================================================
   CBB Projection Model — KenPom + TeamRankings (6 URLs)
   Single-file app.js
   ========================================================= */

/* ---------- CONFIG: paste your URLs ---------- */
const TR_URLS = {
  OFF_EFF:
...
// KenPom publish-to-web CSV (headers may be on row 2)
const KENPOM_URL =
...
const N_SIMS = 1000; // mobile-friendly
const POSS_SD = 3.5;
const PPP_SD = 0.04;

// weights for TR features
const W_EFG = 0.40;
const W_TOV = 0.25;
const W_REB = 0.20;
...
const badge = (t, k) =>
  `<span class="badge ${k === "green" ? "green" : k === "red" ? "red" : "gray"}">${t}</span>`;
...
/* =========================================================
   Robust KenPom loader (scans first 10 rows, fuzzy match)
   ========================================================= */
...
function buildKP(rows) {
  ...
  // find header within first 10 rows
  let headerRow = 0,
    best = -1;
  ...
  if ([iTeam, iAdjO, iAdjD, iAdjT].some((i) => i === -1)) {
    throw new Error("KenPom header not recognized: " + JSON.stringify(header));
  }

  const out = [];
  for (let r = startIdx; r < rows.length; r++) {
    const row = rows[r];
    // skip repeated header lines in the middle
  ...
  if (!out.length) throw new Error("KenPom parsed 0 valid rows (all NaN).");
  return out;
}
...
/* =========================================================
   Deterministic projection + TR adjustments
   ========================================================= */
function findKP(name) {
  const k = teamKey(name);
  return KP.find((r) => teamKey(r.Team) === k) || KP.find((r) => teamKey(r.Team).includes(k));
}
function findTR(name) {
  const k = teamKey(name);
  return TR.find((r) => r._team_key === k) || TR.find((r) => teamKey(r.Team).includes(k));
}
...
self.onmessage = (e) => {
  ...
  function randn(m, s){ const u=Math.random(), v=Math.random();
    const z=Math.sqrt(-2*Math.log(u))*Math.cos(2*Math.PI*v); return m + s*z; }
  ...
  const mean = arr => { let s=0; for(let i=0;i<arr.length;i++) s+=arr[i]; return s/arr.length; };
  const q = (arr, t) => { const b=Array.from(arr).sort((x,y)=>x-y);
    const idx=Math.max(0,Math.min(b.length-1,Math.floor(t*(b.length-1)))); return b[idx]; };
  postMessage({ mA: mean(A), mH: mean(H),
    qA25: q(A,0.25), qA75: q(A,0.75),
    qH25: q(H,0.25), qH75: q(H,0.75),
    homeWinPct: hw / nSims });
}`;
...
runBtn.addEventListener("click", () => {
  try {
    ...
    if (!away || !home || !Number.isFinite(bookSpread) || !Number.isFinite(bookTotal)) {
      alert("Please select teams and enter numeric spread/total."); return;
    }
    ...
    w.onmessage = (ev) => {
      ...
      const bookHomeEdge = -bookSpread; // convert to model sign
      ...
      if (spreadEdge >= SPREAD_EDGE_TH) {
        spreadPlay = `${home} ${bookSpread.toFixed(1)}`;
      } else if (spreadEdge <= -SPREAD_EDGE_TH) {
        spreadPlay = `${away} ${(-bookSpread).toFixed(1)}`;
      }
