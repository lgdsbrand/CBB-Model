...
const KENPOM_URL="https://docs.google.com/spreadsheets/d/e/2PACX-1vRYVL4J6ZbqLvKsS1E32DtBijLaSdrdtermV-Xyno1jSwGHx0m59JAEbq-zVpDztR7CjX-0Ru4jUjMR/pub?gid=351220539&single=true&output=csv";

/* ======= Model Params ======= */
const LEAGUE_AVG_ADJ=105.0;
...
const W_EFG=0.40, W_TOV=0.25, W_REB=0.20;

const TEAM_COL_INDEX=1; // TR sheets: B
...
function findKP(name){ const k=teamKey(name); return KP.find(r=>teamKey(r.Team)===k)||KP.find(r=>teamKey(r.Team).includes(k)); }
function findTR(name){ const k=teamKey(name); return TR.find(r=>r._team_key===k)||TR.find(r=>teamKey(r.Team).includes(k)); }

function baseParams(away,home){
  const A=findKP(away), H=findKP(home);
  ...
  if(TR&&LG){
    const rA=findTR(away), rH=findTR(home);
    ...
    let off_mult_A =Math.pow(eff_anchor_A,0.5)*Math.pow(A_OFF_EFG/Math.max(LG.OFF_EFG,1e-6),W_EFG)*Math.pow((1-A_TOV)/Math.max(1e-6,1-LG.TOV_POSS),W_TOV)*Math.pow(A_OFF_REB/Math.max(LG.OFF_REB,1e-6),W_REB);
    let def_mult_H =Math.pow(eff_anchor_Hd,0.5)*Math.pow(H_DEF_REB/Math.max(LG.DEF_REB,1e-6),W_REB);
    let off_mult_H =Math.pow(eff_anchor_Ho,0.5)*Math.pow(H_OFF_EFG/Math.max(LG.OFF_EFG,1e-6),W_EFG)*Math.pow((1-H_TOV)/Math.max(1e-6,1-LG.TOV_POSS),W_TOV)*Math.pow(H_OFF_REB/Math.max(LG.OFF_REB,1e-6),W_REB);
    let def_mult_A =Math.pow(eff_anchor_Ad,0.5)*Math.pow(A_DEF_REB/Math.max(LG.DEF_REB,1e-6),W_REB);
...
self.onmessage=e=>{
  ...
  for(let i=0;i<nSims;i++){ const p=Math.max(50,randn(poss,possSD)); const a=randn(pppA,pppSD)*p; const h=randn(pppH,pppSD)*p+homeEdge; A[i]=a; H[i]=h; if(h>a) hw++; }
...
/* ======= Saved table ======= */
function loadSaved(){ try{const raw=localStorage.getItem("cbb_saved"); savedGames=raw?JSON.parse(raw):[];}catch{savedGames=[];} renderSaved(); }
  ...
  try{
    ...
    w.onmessage=(ev)=>{
      ...
      saveBtn.onclick=()=>{
        ...
        savedGames.push(row); persistSaved();
      };
      w.terminate();
    };
